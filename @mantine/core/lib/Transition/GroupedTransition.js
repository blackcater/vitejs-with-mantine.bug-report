...
export function GroupedTransition({ transitions, mounted, children, themeOverride, }) {
    const theme = useMantineTheme(themeOverride);
    const duration = Math.max(...Object.keys(transitions).map((transition) => transitions[transition].duration));
    return (React.createElement(Transition, { in: mounted, timeout: duration, unmountOnExit: true, mountOnEnter: true, onEnter: (node) => node.offsetHeight }, (transitionState) => {
